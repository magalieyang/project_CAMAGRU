...
function install($nm){
	...
	if ($connect){
		$requete = 'CREATE TABLE IF NOT EXISTS accountinfos
				...
				NOT NULL DEFAULT "visitor", password VARCHAR(254) NOT NULL, groupe VARCHAR(4) NOT NULL DEFAULT "off", mail VARCHAR(254), date_inscription DATE NOT NULL, avatar VARCHAR(254) NOT NULL DEFAULT "../../other/member.png", username VARCHAR(33) NOT NULL, notif INT DEFAULT 1)';
	...
	// <--INSERTION DE L ADMINISTRATEUR-->
		...
		if (!$res){
			...
			VALUES ("admin", "' . $mdp . '", "on", "user@user.com", CURRENT_DATE, "'. $username .'")';
