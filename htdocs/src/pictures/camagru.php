			...
			<table id= "table">
				...
				<?php
					while ($res2 = $do2->fetch(PDO::FETCH_ASSOC)){
						...
						$id = $res2['id'];
				?>
				<td><button class="minipostcustom" value="<?php echo $res2['path_img'];?>" id="filter_<?php echo $nbr;?>"
						style=" background-image:url('<?php echo $acces; ?>');"
						onclick="displayselected('<?php echo $acces;?>', '<?php echo $id; ?>')"></button></td>
				<?php
				$nbr += 1;
				}
				?>
...
<center>
			...
			<form method="post" action="uploadcam.php">
				...
				<input type="hidden" name="none" value="submit" id="sub" class="sub__" style="display:none;">
			</form>
</center>
			</div>
	...
	<script>
		...
		navigator.getMedia(
			...
			startbutton.addEventListener('click', function(ev){
				...
				document.getElementById('sub').name = "Submit";
