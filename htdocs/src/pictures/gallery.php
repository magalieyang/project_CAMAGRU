				...
				while ($elem = $doreq->fetch(PDO::FETCH_ASSOC)){
					if ($elem){
								...
								?>
									...
									onclick="Select('<?php echo $elem["idpic"]; ?>',
						...
						<div class="infophoto">
							...
							<div id="texxt" style="word-wrap: break-word;"><?php echo $lastd;?></div>
							<?php
						if(isset($_SESSION['user']) && $_SESSION['user'] == $name){
							?>
						<form method="post" action="supprimg.php">
							<button style="background-image:url('../../other/icons8-supprimer-limage-50.png');"
						...
						<?php
							...
							echo "<button id='DEL_' style='display:none' value='none' type='hidden'>";
						}
						?>
						</div>
						<?php
						...
						<script>
							function Select(id, path, titl,desc){
								document.getElementById('photo_s').style.backgroundImage = 'url('+path+')';
								document.getElementById('titt').innerHTML = titl;
								document.getElementById('texxt').innerHTML = desc;
								document.getElementById('DEL_').value = id;
							}
